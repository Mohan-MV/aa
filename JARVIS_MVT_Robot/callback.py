...
from JARVIS_MVT_Robot.database.autofilter_db import is_subscribed, get_file_details
from JARVIS_MVT_Robot.database._utils import get_size
...
@JARVIS_MVT_Robot.on_callback_query()
async def cb_handler(client: JARVIS_MVT_Robot, query):
...
# ---------- 📁 [ | 𝗚𝗘𝗧 𝗙𝗜𝗟𝗘𝗦 | ] 📁 ---------- #
        ...
        elif query.data.startswith("JARVIS_MVT_Robot"):
            ...
            if f_caption is None:
                f_caption = JARVIS.FILE_CAPTIONS.format(mention=query.from_user.mention, title=title, size=size)
            ...
            try:
                if FORCES_SUB and not await is_subscribed(client, query):
                    ...
                    await query.answer('🤖 Check PM, I have Sent Files In Private Message 🤖',show_alert = True)
...
# ---------- 📁 [ | 𝗣𝗠 𝗙𝗜𝗟𝗘𝗦 | ] 📁 ---------- #
            ...
            if FORCES_SUB and not await is_subscribed(client, query):
                await query.answer("I Like Your Smartness, But Don't Be Oversmart. Nakku 😝 😒",show_alert=True)
                return
            ident, file_id = query.data.split("#")
            ...
            for files in filedetails:
                ...
                if CUSTOM_FILE_CAPTION:
                    try:
                        ...
                        f_caption=f_caption
                if f_caption is None:
                    f_caption = JARVIS.FILE_CAPTIONS
                buttons = [[
                  InlineKeyboardButton('🧑‍💻 How To Own 🧑‍💻', url='https://t.me/Mohan_MVT'')
                  ]]                 
...
# ---------- 📁 [ | 𝗠𝗢𝗗𝗨𝗟𝗘𝗦 | ] 📁 ---------- #
            ...
            await query.message.edit(text=JARVIS.HELP_MSG.format(mention=query.from_user.mention), reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
            ...
            await query.message.edit(text=JARVIS.ABOUT_MSG.format(mention=query.from_user.mention, bot_name=bot_info.BOT_NAME, bot_username=bot_info.BOT_USERNAME, dev_name=DEV_NAME), reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
            ...
            await query.message.edit(text=JARVIS.PR0FESS0R_99.format(mention=query.from_user.mention), reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
