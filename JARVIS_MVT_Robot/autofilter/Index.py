...
from JARVIS_MVT_Robot.database.autofilter_db import save_file
...
@JARVIS_MVT_Robot.on_callback_query(Worker.regex(r'^index'))
...
@JARVIS_MVT_Robot.on_message((Worker.forwarded | (Worker.regex("(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$")) & Worker.text ) & Worker.private & Worker.incoming)
...
@JARVIS_MVT_Robot.on_message(Worker.command('setskip') & Worker.user(ADMINS))
